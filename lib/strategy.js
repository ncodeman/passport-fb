...
function verifyWrapper(options, verify) {
  return function passportVerify(req, accessToken, refreshToken, profile, verified) {
    var arity = verify.length;

    if(arity == 5 && options.passReqToCallback) {
      verify(req, accessToken, refreshToken, profile, verified);
    } else if(arity == 4 && !options.passReqToCallback) {
      verify(accessToken, refreshToken, profile, verified);
    } else {
      this.error(new Error('FacebookStrategy: verify callback must take 4 or 5 parameters'));
    }
  }
}
